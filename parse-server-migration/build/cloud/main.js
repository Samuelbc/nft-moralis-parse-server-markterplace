...
Parse.Cloud.afterSave('ItemListed', async (request) => {
    ...
    if (confirmed) {
        ...
        if (alreadyListedItem) {
            ...
            console.log(`Deleted item with tokenId ${request.object.get('tokenId')} at address ${request.object.get('nftAddress')} since the listing is being updated. `);
        }
        // Add new ActiveItem
...
Parse.Cloud.afterSave('ItemCanceled', async (request) => {
    const confirmed = request.object.attributes;
    ...
    if (confirmed) {
        ...
        if (canceledItem) {
            ...
            console.log(`Deleted item with tokenId ${request.object.get('tokenId')} at address ${request.object.get('nftAddress')} since it was canceled. `);
        }
        else {
            console.log(`No item canceled with address: ${request.object.get('nftAddress')} and tokenId: ${request.object.get('tokenId')} found.`);
        }
    }
});
Parse.Cloud.afterSave('ItemBought', async (request) => {
    const confirmed = request.object.attributes;
    console.log(`Marketplace | Object: ${request.object}`);
    if (confirmed) {
        const ActiveItem = Parse.Object.extend('ActiveItem');
        ...
        if (canceledItem) {
            console.log(`Deleting ${request.object.get('objectId')}`);
            await canceledItem.destroy();
            console.log(`Deleted item with tokenId ${request.object.get('tokenId')} at address ${request.object.get('nftAddress')} from ActiveItem table since it was bought.`);
        }
        else {
            console.log(`No item bought with address: ${request.object.get('nftAddress')} and tokenId: ${request.object.get('tokenId')} found`);
        }
    }
});
